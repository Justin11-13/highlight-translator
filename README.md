...
| 已实现 | Appearance 分为 Main window 与 Popup window；主窗口外观独立保存，popup 固定使用 Liquid Glass 原色并保留透明度/模糊/字体设置 |
| 已实现 | 设置窗口与弹窗内 Appearance 面板按当前显示器工作区夹紧，面板关闭恢复原翻译窗口尺寸 |
| 已实现 | 独立 Settings 窗口复用翻译弹窗的 Liquid Glass 外壳、透明度、玻璃颜色、模糊和双语文案 |
...
| 已实现 | Header 移动期间主进程冻结起始宽高，每帧以固定完整 bounds 移动，并拦截意外 resize 事件，避免 DPI/Shape 合成重排传入 renderer |
...
| 已实现 | 原文输入区与译文区采用等高玻璃内容卡；长内容自动换行并在各自内容卡内独立纵向滚动，使用窄型半透明 scrollbar |
...
| 已实现 | 个性化外观：设置窗口可换背景照片；popup 提供字体、文字颜色、透明度、模糊、尺寸和字号设置，背景保持默认 Liquid Glass 原色 |
...
- **Theme presets**：在 Popup window 内只切换英文/中文字体组合与字号，不改变 Liquid Glass 背景、透明度或 Blur。
...
- **字体**：预设列表（系统、微软雅黑、楷体、宋体、Arial、Georgia、Verdana、
  Consolas 等宽）+ 字号 / 缩放 / 透明度 / 模糊滑杆；模糊值最低为 8px。
...
- [x] 已验证：TypeScript 类型检查和 Electron 构建（`npm run typecheck`、`npm run build`）
...
1. **本机 WDAC 代码完整性策略**（企业签名级别要求）拦截无签名 exe/dll：
   ...
   - 打包/安装阶段首次 spawn 新安装器可能被云信誉短暂拦截，重试即可。
